...
class _SpringySliderState extends State<SpringySlider>
  ...
  SpringySliderController sliderControlder;

  @override
  void initState() {
    super.initState();

    sliderControlder = SpringySliderController(
      sliderPercent: 0.5,
      vsync: this,
    )..addListener(() {
        setState(() {});
  ...
  Widget build(BuildContext context) {
    return SliderDrager(
      ...
      sliderController: sliderControlder,
      child: Stack(
        children: <Widget>[
          SliderMarks(
          ...
          SliderGoo(
            sliderControler: sliderControlder,
            paddingTop: paddingTop,
            paddingBottom: paddingBottom,
            child: SliderMarks(
              markCount: widget.markCount,
          ...
          //   paddingBottom: paddingBottom,
          //   paddingTop: paddingTop,
          //   sliderPercent: sliderControlder.state == SpringySliderState.dragging
          //       ? sliderControlder.draggingPercent
          //       : sliderPercent,
          // ),
        ],
      ),
...
class SliderMarksPainter extends CustomPainter {
  final double largeMarkWidth = 50.0;
  final double smallMarkWidth = 20.0;
  ...
  @override
  bool shouldRepaint(SliderMarksPainter oldDelegate) {
    return true;
  }
}
...
class SliderClipper extends CustomClipper<Path> {
  ...
  Path _clipDragging(Size size) {
    ...
    final thichkeningFactor = excessDrag * height * 0.05;
    final controlPointWidth =
        (200.0 * thichkeningFactor).abs() + baseControlPointWidth;
    ...
    curve.quadraticBezierTo(crestPoint.x - controlPointWidth, crestPoint.y,
  ...
  Path _clipSpringging(Size size) {
    var compositePath = Path();
    ...
    rightCurve.quadraticBezierTo(crestPoint.x - controlPointWidth, crestPoint.y,
    ...
    leftCurve.quadraticBezierTo(troughPoint.x - controlPointWidth,
        troughPoint.y, leftPoint.x, leftPoint.y);

    leftCurve.moveTo(troughPoint.x, troughPoint.y);
    leftCurve.quadraticBezierTo(troughPoint.x + controlPointWidth,
        troughPoint.y, centerPoint.x, centerPoint.y);
...
class SliderGoo extends StatelessWidget {
  ...
  final double paddingBottom;
  final Widget child;
  final SpringySliderController sliderControler;

  SliderGoo({
    this.child,
    this.paddingTop,
    this.paddingBottom,
    this.sliderControler,
  });
  ...
  Widget build(BuildContext context) {
    return ClipPath(
      clipper: SliderClipper(
        sliderController: sliderControler,
        paddingTop: paddingTop,
        paddingBottom: paddingBottom,
      ),
      child: child,
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, contrains) {
        final height = contrains.maxHeight - paddingTop - paddingBottom;
...
class SliderDrager extends StatefulWidget {
  final Widget child;
  final double paddingTop;
  final double paddingBottom;
  final SpringySliderController sliderController;
  ...
  @override
  _SliderDragerState createState() => _SliderDragerState();
}

class _SliderDragerState extends State<SliderDrager> {
  double startDragY;
  ...
  void _onPanUpdate(DragUpdateDetails details) {
    ...
    final sliderHeight =
        context.size.height - widget.paddingBottom - widget.paddingTop;
    ...
    widget.sliderController.draggingPercents =
        Offset(draggingHorizontalPercent, startDragPercent + dragPercent);
  }
